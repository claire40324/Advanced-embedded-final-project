...
void BSP_LCD_Message (int device, int line, int col, char *string, unsigned int value){
  ...
  uint16_t y = 0;
	...
	//seperate screen into two parts using device variable
  if(device == 1)
    y = 12;
    else if(device == 0 && line < 12 && line > 0)
    y = line; 
		
		//set x position for string column values and StY for number row values
    x = (uint16_t)col;
    StY = 12;
  ...
  if(col == 1)
    ...
    StX = 15;
		...
		//set position of the cursor to print numbers
	BSP_LCD_SetCursor(StX, StY);
		
		//print values
  BSP_LCD_OutUDec4(value, ST7735_WHITE);

		//print string
  BSP_LCD_DrawString(x, y, string, ST7735_WHITE);  
}

void BSP_LCD_String(int line, int col, char *string)
{
	BSP_LCD_DrawString(col, line, string, ST7735_WHITE);
}

void BSP_LCD_Value(int line, int col,unsigned int value)
{
...
//					color		specifies the color of the crosshair
// outputs: none
void BSP_LCD_DrawCrosshair(int16_t x, int16_t y, uint16_t color) {

  BSP_LCD_DrawFastVLine(x, y - (128 / 24), (128 / 12), color);
  BSP_LCD_DrawFastHLine(x - (128 / 24), y, (128 / 12), color);
}
